...
class BeneficiariosBloc {
  ...
  final _beneficiariosController =
      BehaviorSubject<List<BeneficiariosGeneral>>();
  final _beneficiariosAfiliadosController =
      BehaviorSubject<List<BeneficiariosGeneral>>();

  Stream<List<BeneficiariosGeneral>> get beneficiariosControllerStream =>
      _beneficiariosController.stream;
  Stream<List<BeneficiariosGeneral>>
      get beneficiariosAfiliadosControllerStream =>
          _beneficiariosAfiliadosController.stream;
  ...
  Future<List<BeneficiariosGeneral>> benefis(String idPersona) async {
    ...
    if (todosBeneficiarios.length > 0) {
      ...
      if (listBeneficioFallecido.length > 0) {
        ...
        beneficiariosGeneral.texto =
            'Beneficiarios con derecho al fondo de cesantía y bolsa de jubulación';
        ...
        for (var i = 0; i < listBeneficioFallecido.length; i++) {
          ...
          bene.porcentaje =
              double.parse(listBeneficioFallecido[i].porcentaje ?? '')
                  .toInt()
                  .toString();
      ...
      if (listFallecido.length > 0) {
        ...
        beneficiariosGeneral.texto =
            'Derecho como afiliado al cobro del fondo de funerales';
        ...
        for (var i = 0; i < listFallecido.length; i++) {
          ...
          bene.porcentaje = double.parse(listFallecido[i].porcentaje ?? '')
              .toInt()
              .toString();
      ...
      if (listFuneral.length > 0) {
        ...
        beneficiariosGeneral.tipo = '0';
        beneficiariosGeneral.texto =
            'A mi fallecimiento el fondo de funerales debe entregarse a:';
        ...
        for (var i = 0; i < listFuneral.length; i++) {
          ...
          bene.porcentaje =
              double.parse(listFuneral[i].porcentaje ?? '').toInt().toString();
          bene.observacion = listFallecido[i].observaciones;
...
class Beneficiarios {
  Beneficiarios(
      {this.porcentaje, this.nombre, this.parentesco, this.observacion});

    ...
    try {
      ...
      if (decodedData.length > 0) {
        for (int i = 0; i < decodedData.length; i++) {
          ...
          beneficiarios.idBeneficiario = decodedData[i]['id_afiliado_beneficiario'].toString();
          beneficiarios.idPersona = decodedData[i]['id_persona'].toString();
          beneficiarios.nombre = decodedData[i]['Nombre'].toString();
          beneficiarios.gradoParentesco = decodedData[i]['grado_de_parentesco'].toString();
          ...
          beneficiarios.beneficioFallecimiento = decodedData[i]['beneficio_por_fallecimiento'].toString();
          beneficiarios.idGradoParentesco = decodedData[i]['id_grado_de_parentesco_dgrado_de_parentesco'].toString();
          beneficiarios.defuncion = decodedData[i]['defuncion'].toString();
          beneficiarios.cartaDeclaratoria = decodedData[i]['Carta_declaratoria'].toString();
          beneficiarios.observaciones = decodedData[i]['Observaciones'].toString();
